...
public class UserServiceTest {
	...
	public void testAddUser() throws Exception {
		...
		User user = new User();
		user.setUserId("rachaelId");
		user.setUserName("rachaelName");
		user.setPassword("rachaelPw");
		user.setSsn("1111112222222");
		user.setPhone("555-555-5555");
		user.setAddr("newyork");
		user.setEmail("rachael@test.com");
		
		userService.addUser(user);
		
		user = userService.getUser("rachaelId");

		//==> console 확인
		//System.out.println(user);
		
		//==> API 확인
		Assert.assertEquals("rachaelId", user.getUserId());
		Assert.assertEquals("rachaelName", user.getUserName());
		Assert.assertEquals("rachaelPw", user.getPassword());
		Assert.assertEquals("555-555-5555", user.getPhone());
		Assert.assertEquals("newyork", user.getAddr());
		Assert.assertEquals("rachael@test.com", user.getEmail());
	}
...
//			user.setEmail("test@test.com");
		...
		Assert.assertEquals("rachaelId", user.getUserId());
		Assert.assertEquals("rachaelName", user.getUserName());
		Assert.assertEquals("rachaelPasswd", user.getPassword());
		Assert.assertEquals("555-555-5555", user.getPhone());
		Assert.assertEquals("newyork", user.getAddr());
		Assert.assertEquals("rachael@test.com", user.getEmail());
